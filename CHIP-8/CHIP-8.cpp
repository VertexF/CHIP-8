...
#define LIST_INSTRUCTIONS(o) \
		...
		o("ret",			"00EE",	u==0x0 && nnn==0xEE	, PC = Stack[SP-- % 16]					)\
		o("jp N",			"1nnn",	u==0x1				, PC = nnn								)\
		o("jp v0,N",		"Bnnn",	u==0xB				, PC = nnn + V[0]						)\
		o("call N",			"2nnn",	u==0x2				, Stack[++SP % 16] = PC; PC = nnn		)\
		...
		o("add i,vX",		"Fx1E",	u==0xF && kk==0x1E	, p = (I & 0xFFFF) + Vx; VF = p >> 12; I = p	)\
...
int main(int argc, char** argv)
{
	...
	// Create a screen.
	SDL_Window* window = SDL_CreateWindow("CHIP-8 Computer", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, W * 4, H * 6, SDL_WINDOW_RESIZABLE | SDL_WINDOW_FULLSCREEN_DESKTOP);
